                           ...
                           unsigned depth, unsigned maxlog)
{
    ...
    for (unsigned logn = 0; logn <= maxlog && ok; logn++) {
        ...
        for (size_t i = 0; i < p.len; i++) {
            mp_limb_t x = (mp_limb_t)(i * 6364136223846793005ULL + 1442695040888963407ULL);
        ...
        for (size_t i = 0; i < p.len; i++)
            ok &= sc_fft_equal(sc_fft_entry(a, i, &m), sc_fft_entry(b, i, &m), &m);
        free(work);
        free(a);
        sc_fft_plan_clear(&p);
    }
...
int main(void)
{
    if (!test_roundtrips(1, 64, 2, 7, 7) ||
        !test_roundtrips(15, 27, 440564289, 27, 10) || !test_convolution()) {
